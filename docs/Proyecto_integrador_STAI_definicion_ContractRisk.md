...
El objetivo final del proyecto es integrar distintas funciones de análisis que permitan generar alertas sobre posibles riesgos contractuales. Estas alertas servirán para organizar y enfocar la revisión, pero no constituirán una acusación ni una determinación automática de fraude, corrupción o ilegalidad. La decisión y la interpretación final siempre corresponderán a una persona experta.

La tarea desarrollada en M1 es una primera función principal dentro de ese objetivo general: identificar si la descripción de un contrato contiene información suficiente para comprender razonablemente qué se está contratando. Una descripción vaga, incompleta o ambigua puede ser una señal útil para priorizar una revisión, aunque por sí sola no demuestre que exista una irregularidad.

Para esta función de M1, la salida tiene dos clases:
...
Por tanto, `REQUIERE_REVISION` representa únicamente una alerta de insuficiencia descriptiva dentro de ContractRisk; no significa que el contrato sea fraudulento, corrupto o ilegal.
...
- Macro Recall: 0,712.
- Macro F1: 0,749.
- Matriz de confusión: `[[14, 17], [3, 106]]`, con orden `[REQUIERE_REVISION, SUFICIENTE]`.

El modelo superó ampliamente el baseline trivial, demostrando aprendizaje real, pero quedó 0,089 puntos de Macro F1 por debajo de TF-IDF. Detectó 14 de 31 revisiones y dejó escapar 17; en cambio, reconoció 106 de 109 textos suficientes. Su comportamiento fue conservador: cuando emitió una alerta de revisión normalmente tuvo razón —precisión 0,824—, pero generó pocas alertas para esa clase —recall 0,452—.

La corrida final se ejecutó en una GPU Tesla T4. El mejor checkpoint apareció en la época 7 con Macro F1 0,748563. Las métricas mejoraron hasta la última evaluación conservada, por lo que `load_best_model_at_end=True` mantuvo ese checkpoint.
